...
- 로지스틱 회귀모형은 선형 회귀분석과 달리 반응변수가 `범주형 데이터`인 경우에 사용되는 기법
- 새로운 설명변수의 값이 주어질 때 반응변수의 각 범주에 속할 확률이 얼마인지를 추정하고 추정확률을 분류기준값에 따라 `분류`하는 목적으로 사용
- 이진값을 가지는 반응변수를 여러 설명변수를 이용해 회귀식의 형태로 예측하는 모형

![image](https://github.com/AlishaKang/TIL/assets/145949899/91723ef3-5066-4b11-9671-ebca2273c048)

- 반응변수Y가 관심 범주 1에 속할 확률 p인 베르누이 확률분포를 따름, 비스듬한 S곡선(Sigmoid함수) 형태 가짐
- n개의 표본 자료가 주어진다면 최대우도추정법, 경사하강법 등을 이용해 가장 적합한 곡선 함수를 추정 


## 로지스틱 회귀모형의 분리경계면

- 로지스틱 회귀모형은 선형의 결정경계를 가짐
- 이 경계선을 기준으로 1에 속하는지 0에 속하는지가 나누어짐 

## 로지스틱 회귀와 오즈비 (odds ratio)

- 나머지 변수는 모두 고정시킨 상태에서 한 변수 X만 1만큼 증가시켰을 때, 변화하는 odds의 비율은 e임을 알 수 있음
- X가 1만큼 증가하면 성공(관심범주, Y=1)에 대한 오즈가 exp(B)배 변화함
```
B > 0 : 관심범주에 속할 확률 증가
